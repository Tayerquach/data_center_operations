...
def check_datacenter_slots_size_constraint(fleet):
    ...
    constraint = test.any()
    if constraint:
        print(slots['slots_size'], slots['slots_capacity'])
...
def get_evaluation(solution, 
    ...
    metrics = {
        ...
        'O': [],
        'D': {
            (sg, ls): [] for sg in get_known('server_generation') for ls in get_known('latency_sensitivity')
        },
        'Z': {
            (sg, ls): [] for sg in get_known('server_generation') for ls in get_known('latency_sensitivity')
        },
    ...
    for ts in tqdm(range(1, time_steps+1)):
        ...
        if FLEET.shape[0] > 0:
            ...
            for dc in get_known('datacenter_id'):
                if dc in slots.index:
                    metrics['SLOTS'][dc].append(slots.loc[dc, 'slots_size'] / slots.loc[dc, 'slots_capacity'])
                else:
            ...
            for sg in get_known('server_generation'):
                for ls in get_known('latency_sensitivity'):
                    ...
                    metrics['D'][(sg, ls)].append(D[ls].get(sg, default=0))
    ...
    rich.print(f"Utilization : {sum(metrics['U']):0,.2f}")
    rich.print(f"Lifespan : {sum(metrics['L']):0,.2f}")
    rich.print(f"Profit : {sum(metrics['P']):0,.2f}")
    rich.print(f"- Revenue : {sum(metrics['R']):0,.2f}")
    rich.print(f"- Cost : {sum(metrics['C']):0,.2f}")
    
    rich.print(f"Objective : {sum(metrics['O']):0,.2f}")
